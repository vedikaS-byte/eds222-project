...
According to the `betareg` CRAN project, a variable cannot take exact values of 0 or 1. Therefore, a zero-adjustment is needed to slightly decrease values of 1 to just below 1 and slightly increase values of 0 to just above 0. In this analysis, I applied the zero-adjustment formula from the `betareg` vignette: $y_{\text{adj}} = \frac{y \cdot (n - 1) + 0.5}{n}$, where n is the sample size (Smithson and Verkuilen, 2006).
...
Recall the definition of the beta distribution used in the model:
